...
class Auth_Driver_db extends Auth_Driver
{
	...
	protected /*bool*/ function throttleByAccount($username)
	{
		...
		if($throttleuser === null) {
			...
			throw new AuthException(AuthExceptionType::INVALID_USERNAME, "You entered an invalid username/password combination.");
			return false;
		}
		...
		if($delay > 0 && $query->row['last'] + $delay > time()) {
			throw new AuthException(AuthExceptionType::USER_THROTTLED, "You must wait before trying again.");
			return false;
		}
	}
	...
	public /*bool*/ function attempt($username, $password, $persistent=false, array $extra=array()) /*throws AuthException*/
	{
		...
		if($delay > 0 && $query->row['last'] + $delay > time()) {
			throw new AuthException(AuthExceptionType::IP_THROTTLED, "You must wait before trying again.");
			return false;
		}
		...
		if($user === null) {
			...
			throw new AuthException(AuthExceptionType::INVALID_PASSWORD, "You entered an invalid username/password combination.");
			return false;
		}
		...
		if($user->banned()) {
			...
			throw new AuthException(AuthExceptionType::USER_BANNED, "You cannot log into a banned account.");
			return false;
		}
	...
	public /*bool*/ function check($password) /*throws AuthException*/
	{
		...
		if(!$this->loggedIn())
			throw new AuthException(AuthExceptionType::NO_USER, "The session is not logged in.");
		...
		if(!$this->user->checkPassword($password)) {
			...
			throw new AuthException(AuthExceptionType::INVALID_PASSWORD, "You entered an incorrect password.");
			return false;
		}
