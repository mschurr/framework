	...
	public function __construct($session = null, $driver=null)
	{
		if(is_null($driver)) {
			$driver = Config::get('auth.driver', function(){
				throw new AuthException(AuthExceptionType::MISCONFIGURED);
			});
		}
		if(is_string($driver)) {
...
{
	protected $type = AuthExceptionType::UNKNOWN;
...
/**
 * An enumeration of login failure reasions.
 */
class AuthExceptionType {
	const NOT_SUPPORTED = "NOT_SUPPORTED";
	const NOT_IMPLEMENTED = "NOT_IMPLEMENTED";
	const MISCONFIGURED = "MISCONFIGURED";
	const INVALID_USERNAME = "INVALID_USERNAME";
	const INVALID_PASSWORD = "INVALID_PASSWORD";
	const USER_THROTTLED = "USER_THROTTLED";
	const IP_THROTTLED = "IP_THROTTLED";
...
abstract class Auth_Driver
{
	...
	public static /*void*/ function terminateAllSessionsForUser(User_Provider $user) { throw new AuthException(AuthExceptionType::NOT_IMPLEMENTED); }

	/* Returns the failed login attempts for a given user since a unix timestamp. */
	public static /*int*/ function countFailedLoginsForUserSince(User_Provider $user, $time=0) { throw new AuthException(AuthExceptionType::NOT_IMPLEMENTED); }
	public static /*array<Auth_Attempt>*/ function failedLoginsForUserSince(User_Provider $user, $time=0, $limit=50, $offset=0) { throw new AuthException(AuthExceptionType::NOT_IMPLEMENTED); }

	/* Returns the all login attempts for a given user since a unix timestamp. */
	public static /*int*/ function countLoginsForUserSince(User_Provider $user, $time=0) { throw new AuthException(AuthExceptionType::NOT_IMPLEMENTED); }
	public static /*array<Auth_Attempt>*/ function loginsForUserSince(User_Provider $user, $time=0, $limit=50, $offset=0) { throw new AuthException(AuthExceptionType::NOT_IMPLEMENTED); }

	/* Returns the login attempts reported fradulent for a given user since a unix timestamp. */
	public static /*int*/ function countFraudulentLoginsForUserSince(User_Provider $user, $time=0) { throw new AuthException(AuthExceptionType::NOT_IMPLEMENTED); }
	public static /*array<Auth_Attempt>*/ function fraudulentLoginsForUserSince(User_Provider $user, $time=0, $limit=50, $offset=0) { throw new AuthException(AuthExceptionType::NOT_IMPLEMENTED); }
